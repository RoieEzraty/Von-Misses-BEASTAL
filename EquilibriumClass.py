...
class EquilibriumClass:
    ...
    def potential_energy_constrained_ends(self, free_dof_displacement: jnp.ndarray, time: float, 
                                          ...
                                          impulse_data: jnp.ndarray) -> jnp.ndarray:
    ...
    def rhs(self, state: jnp.ndarray, time: float, m1s: jnp.ndarray, m2s: jnp.ndarray, 
        ...
        force = -self.force_fn(displacement, time, stiffness_vals, timepoints, impulse_data)
        acceleration = (force - damping) / mass_vals
        return jnp.stack((velocity, acceleration))

    def solve(self, state: "StateClass", supervisor: "SupervisorClass") -> jnp.ndarray:
        ...
        self.sol_free = odeint(self.rhs, state.state0_local, supervisor.timepoints, state.m1s_free, 
                               ...
                               supervisor.impulse_data, rtol=supervisor.rtol, atol=supervisor.atol,
